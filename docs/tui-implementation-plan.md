...
- 🟡 **Phase 1 Day 7-8 — RPC command catalog** (next): `hello`, `helloAck` (already shipped), `agent.list/spawn/close/reassignSeat`, `layout.get/save/import/export/setDefault`, `assets.list/requestBlob/addDir/removeDir`, `settings.get/set`, `hooks.toggle`, `daemon.shutdown`, `subscribe`. Vitest per method.
...
1. **Day 1** — Cargo workspace. Pin: `ratatui 0.30`, `ratatui-crossterm 0.1`, `crossterm 0.29`, `wezterm-term 0.22`, `tokio 1.47`, `vte 0.13`, `tachyonfx 0.24`, `arboard 3`, `directories 6`.
