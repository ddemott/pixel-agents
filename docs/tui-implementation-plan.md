...
- ✅ **Phase 2 Day 13-14 — Reconnect logic + supervisor handoff.** `client/src/reconnect.rs`: `ReconnectState` (probe schedule: 250 ms / 1 s / 3 s loop; `on_failure`, `is_due`, `should_fork`, `elapsed_secs`) + `try_fork_daemon()` (spawns `pixel-agents-daemon` binary; no-op until binary lands). `app::run` changed to take `ClientCapabilities` instead of `DaemonConn`; starts with `reconnect = new_immediate()` so initial connect flows through the same retry path. `conn: Option<DaemonConn>` in event loop; `recv_from_conn` helper pends forever when disconnected; reconnect timer fires as a `sleep_until` arm in `tokio::select!`. On EOF/error: `on_disconnected` clears pending RPCs + sets 250 ms retry. On reconnect: `on_connected(boot_id)` clears agents + resets PTY focus if `bootId` changed (daemon restart). Fork attempt fires after second probe failure. `draw_main` shows reconnect overlay in all states. `DaemonConn` gains `pub boot_id: String` field. 15 new tests (9 reconnect schedule/fork, 5 on_connected/on_disconnected, 1 boot-id-change focus reset); 93/93 passing.
- ✅ **Phase 2 Day 9-12 — Focus FSM + chrome + agent.list/spawn.** `client/src/focus.rs`: `FocusMode` enum (`Office | PtyAgent(i32) | Editor | Modal`); `tab_press()` returns `TabOutcome` (Focus/PassThroughToPty/NoOp); `normalize()` maps sub-agent negative IDs to parent. `client/src/keymap.rs`: `Keymap` loads `~/.pixel-agents/keymap.toml` with defaults (Ctrl+Alt+Q quit, Ctrl+Alt+L toggle layout, Ctrl+Alt+O focus office); `parse_chord()` parses `"ctrl+alt+q"` → `(KeyModifiers, KeyCode)`; `apply_toml()` overrides any action's bindings, removing old bindings first. `client/src/agents.rs`: `AgentSnapshot` (serde `Deserialize`, camelCase remaps); `AgentStatus` enum (`Idle/Active(tool)/Waiting/Exited`); `AgentState` composite; `parse_agent_list()` for `agent.list` response. `client/src/chrome.rs`: `ChromeAction` enum; `HitRect`; `hit_test()` by `(col,row)`; `split_areas()` → `Areas { status_bar, main, toolbar }`; `draw()` renders status bar (focus label left, zoom controls right) + toolbar (`[+ Agent] [Layout] [Settings]`) and returns `Vec<HitRect>`. `client/src/app.rs` rewritten: `AppState` (focus, agents, zoom 1–10, req_id monotonic, pending `HashMap<u32, PendingKind>`, hit_rects, status toast); `run()` sends `subscribe` + `agent.list` on boot; `Inbound::Res` routed through `PendingKind` dispatch; `Inbound::Evt` handles `agent.created/exited/statusChanged`; mouse `Down(Left)` hit-tests chrome; Tab triggers `tab_press()`; `q`/Ctrl-C quit; bracketed paste forwarded in PtyAgent (no-op until Phase 4). `tui.rs` gains `EnableMouseCapture + EnableBracketedPaste` on init; `DisableMouseCapture + DisableBracketedPaste` on Drop. `daemon/mod.rs`: `wire` + `framing` submodules promoted to `pub mod`. `Cargo.toml` adds `toml = "0.8"`. 42 new tests; 78/78 total, zero warnings.
