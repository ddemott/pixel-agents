...
| `daemon/`     | Standalone daemon (TUI port): UDS RPC (channel-mux framing, hello handshake, method dispatcher), broadcast event bus with per-conn topic filter, writer-tagged layout / config / agents persistence with cross-window watchers, hook HTTP server, hook installer, test suite (Vitest) |
