...
Deep reference for the file tree and per-subsystem internals. `CLAUDE.md` holds the always-loaded essentials (vocabulary, lessons, build, constraints, decisions) and links here. Consult this when touching a specific subsystem.
...
client/                       — Rust TUI client (TUI port Phase 2+; Rust 1.95+, Cargo workspace)
  ...
  src/
    ...
    render/
      ...
      colorize.rs             — HSL "Adjust" port of colorize.ts adjustSprite: rgb_to_hsl / hsl_to_rgb / adjust_rgba (in-place RGBA hue/sat/bright/contrast shift; transparent px untouched). Chars use hue-only {h,0,0,0}. Photoshop Colorize mode (floor tiles) not ported yet.
      bubbles.rs              — Day 19 speech bubbles: Rust copies of bubble-permission/bubble-waiting JSON (11×13) built once to RGBA via OnceLock. Waiting-bubble fade skipped at cell tiers (no per-pixel alpha).
      matrix.rs               — Day 20 spawn/despawn effect: matrix_effect_seeds + render_matrix_frame (port of renderMatrixEffect → 16×32 RGBA composite; per-column staggered head sweep + green trails + flicker hash; deterministic). Empty-space trail alpha <128 thresholds out at cell tiers.
    ...
    pty/mod.rs                — Phase 4 PTY hosting (client): PtyTerminal wraps tattoy_wezterm_term::Terminal (PtyConfig impl TerminalConfiguration → default ColorPalette; writer = sink, answerback discarded for now). advance(bytes) feeds the parser; render_into(buf, area) clears to default bg then paints visible rows (lines_in_phys_range, since visible_lines() is test-only) with palette-resolved fg/bg + inverts the cursor cell; resize/size for later resize-follow. Wired in app.rs: AppState.terminals map (per-agent, keyed by stream id = agent id), Frame::PtyOut → ingest_pty (ingests ALL agents so focus switch is instant), draw_main renders the focused agent's grid over the main area (all tiers), dropped on agent.exited. `encode_key(KeyCode, KeyModifiers)` maps a key press to PTY bytes (chars/ALT-prefix, Ctrl→C0, Enter/Tab/Backspace/Esc, arrows+nav CSI; unmapped→None); when PTY-focused, app.rs `handle_event` forwards keys + paste via `send_pty_input` (`pty.input` NDJSON RPC, base64, fire-and-forget), all keys except Ctrl+Alt app escapes. Resize-follow: each tick `pty_grid_size` (chrome main minus border) is compared to the focused term's size; on change `PtyTerminal::resize` + `send_pty_resize` (`pty.resize` RPC) SIGWINCHes the child. Scrollback: `render_into(buf, area, scroll_offset)` shifts the phys-range window into history (cursor hidden while scrolled), `max_scroll()` clamps; app.rs per-agent `scroll_offset` map, PgUp/PgDn scroll the client view (not forwarded), typing snaps to bottom. `term_size(cols,rows)` reports non-zero cell-pixel dims (8×16) so inline image escapes can't divide-by-zero panic the fork's image placement. `encode_paste(text, bracketed)` wraps a paste in `ESC[200~`…`ESC[201~` when the focused term has DECSET 2004 enabled (`bracketed_paste_enabled()`, via `Terminal`→`TerminalState` Deref), de-fanging any embedded end-marker; raw otherwise. app.rs `Event::Paste` queries the flag, encodes, snaps scroll to bottom, forwards via `pty.input`. Slices done: 1 (ingest+render), 2 (input), 3 (resize-follow), 4 (scrollback), 5 (bracketed-paste). Later: mouse-mode forwarding, click-to-focus, graceful death, focus-arbitration RPCs. PtyByteTap strip half dropped (parser ignores image escapes safely); passthrough half blocked on image-tier live wiring. Gaps: answerback discarded, arrows ignore DECCKM, inbound 0x03 binary PTY frames unused daemon-side, terminal::size() polled per frame.
